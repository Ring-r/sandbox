...
namespace MoveInCells
{
    public class Entities
    {
        ...
        private readonly float maxV = 3.0f;
        ...
        private void RunEvent(Entity entity, Entity entityNext, ref bool isRun)
        {
        ...
        private void FreezeEvent(Entity entity, Entity entityNext, ref bool isRun)
        {
        ...
        private void UpdateScore()
        {
            ...
            foreach (Entity entity in this.entities)
            {
                ...
                {
                    case State.Run:
                        this.NearEntitiesEvent(entity, this.RunEvent);
                        break;
                    case State.Freeze:
                        if (this.NearEntitiesEvent(entity, this.FreezeEvent))
                        {
                            entity.State = State.Catch;
                        }
                        break;
                    case State.Catch:
                        this.NearEntitiesEvent(entity, this.CatchEvent);
                        break;
                }
        ...
        public void Draw(Graphics g, float width, float height)
        {
            ...
            double rg_rad = 1.5f * (float)Math.PI - mainEntity.A;
            foreach (Entity entity in this.entities)
            {
                ...
                #region Recalculate local coordinates.

                float xl = entity.X - mainEntity.X;
                float yl = entity.Y - mainEntity.Y;

                if (xl < -areaSize_2)
                    xl += areaSize;
                else if (xl > areaSize_2)
                    xl -= areaSize;

                if (yl < -areaSize_2)
                    yl += areaSize;
                else if (yl > areaSize_2)
                    yl -= areaSize;

                float xlr = (float)(xl * Math.Cos(rg_rad) - yl * Math.Sin(rg_rad));
                float ylr = (float)(xl * Math.Sin(rg_rad) + yl * Math.Cos(rg_rad));

                #endregion Recalculate local coordinates.

                g.FillEllipse(brush, xlr - entity.R, ylr - entity.R, 2 * entity.R, 2 * entity.R);
                g.DrawEllipse(Pens.Black, xlr - entity.R, ylr - entity.R, 2 * entity.R, 2 * entity.R);
                g.DrawString(entity.Score.ToString(), font, Brushes.Black, xlr + entity.R, ylr + entity.R);
                #region For testing.
                if (entity.V > 0)
                {
                    ...
                    g.DrawLine(Pens.Black, xlr, ylr, xlr + vx, ylr + vy);
                }
                #endregion For testing.
                #region For testing.
                //                if (entity.State != State.Run)
